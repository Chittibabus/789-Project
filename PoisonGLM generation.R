...
setwd("place the path you want the file to be generated")

Alley.Lights<- read.csv(file="place the generated cvs files from python here", head=TRUE)

Street.Lights.OneOut<- read.csv(file="place the generated cvs files from python here", head=TRUE)

Street.Lights.AllOut<- read.csv(file="place the generated cvs files from python here", head=TRUE)


Alley.Lights$CommunArea <- as.character(Alley.Lights$CommunArea)
Street.Lights.OneOut$CommunArea <- as.character(Street.Lights.OneOut$CommunArea)
Street.Lights.AllOut$CommunArea <- as.character(Street.Lights.AllOut$CommunArea)

Alley.Lights$CommunArea[which(nchar(Alley.Lights$CommunArea)==4)] <- as.numeric(substr(Alley.Lights$CommunArea[which(nchar(Alley.Lights$CommunArea)==4)],3,3))
Alley.Lights$CommunArea[which(nchar(Alley.Lights$CommunArea)==5)] <- as.numeric(substr(Alley.Lights$CommunArea[which(nchar(Alley.Lights$CommunArea)==5)],3,4))
Street.Lights.OneOut$CommunArea[which(nchar(Street.Lights.OneOut$CommunArea)==4)] <- as.numeric(substr(Street.Lights.OneOut$CommunArea[which(nchar(Street.Lights.OneOut$CommunArea)==4)],3,3))
Street.Lights.OneOut$CommunArea[which(nchar(Street.Lights.OneOut$CommunArea)==5)] <- as.numeric(substr(Street.Lights.OneOut$CommunArea[which(nchar(Street.Lights.OneOut$CommunArea)==5)],3,4))
Street.Lights.AllOut$CommunArea[which(nchar(Street.Lights.AllOut$CommunArea)==4)] <- as.numeric(substr(Street.Lights.AllOut$CommunArea[which(nchar(Street.Lights.AllOut$CommunArea)==4)],3,3))
Street.Lights.AllOut$CommunArea[which(nchar(Street.Lights.AllOut$CommunArea)==5)] <- as.numeric(substr(Street.Lights.AllOut$CommunArea[which(nchar(Street.Lights.AllOut$CommunArea)==5)],3,4))


Summary.Table.Alley  <- data.frame(matrix(ncol = 4, nrow = 8), row.names = c("All Crimes (No misleading Data)", 
                                                                              "Theft", "Narcotics", "Battery", "Criminal Damage", "Motor Vehicle Theft", 
                                                                               "Robbery", "Assault"))
Summary.Table.OneOut <- data.frame(matrix(ncol = 4, nrow = 11), row.names = c("All Crimes (No misleading Data)", 
                                                                              "Theft", "Narcotics", "Battery", "Criminal Damage", "Motor Vehicle Theft", 
                                                                              "Robbery", "Assault", "Burglary", "Homicide", "Deceptive Practice"))
Summary.Table.AllOut <- data.frame(matrix(ncol = 4, nrow = 11), row.names = c("All Crimes (No misleading Data)", 
                                                                              "Theft", "Narcotics", "Battery", "Criminal Damage", "Motor Vehicle Theft", 
                                                                              "Robbery", "Assault", "Burglary", "Homicide", "Deceptive Practice"))
Summary.Table.Alley  <- rename(Summary.Table.Alley,  c("A"="Before", "B"="During", "C"="After"))
Summary.Table.OneOut <- rename(Summary.Table.OneOut, c("A"="Before", "B"="During", "C"="After"))
Summary.Table.AllOut <- rename(Summary.Table.AllOut, c("A"="Before", "B"="During", "C"="After"))
...
Alley.Lights.Pois <- Alley.Lights.Pois[, c("Type","DateCreated", "DateCompleted", "Location", "x_coord", "y_coord", "zip_code", "ward",
                                           "police_district", "CommunArea", "Time", "AllCrimes", "Theft", "Narcotics", "Battery", "CriminalDamage", 
...
Street.Lights.OneOut.Pois <- Street.Lights.OneOut.Pois[,c("Type","DateCreated", "DateCompleted", "Location", "x_coord", "y_coord", "zip_code", "ward",
                                          "police_district", "CommunArea", "Time", "AllCrimes", "Theft", "Narcotics", "Battery", "CriminalDamage", 
...
Street.Lights.AllOut.Pois <- Street.Lights.AllOut.Pois[,c("Type", "DateCreated", "DateCompleted", "Location", "x_coord", "y_coord", "zip_code", "ward",
                                                          "police_district", "CommunArea", "Time", "AllCrimes", "Theft", "Narcotics", "Battery", "CriminalDamage", 
                                                          "MotorVehicleTheft", "Robbery", "Assault", "Burglary", "Homicide", "DeceptivePractice", "Duration", "OutageDuration")]
